...
Sys.Application.add_load(LoadPageGate);

var slCookieName = "mtps_SL";
var numberOfDaysToPostpone = 7;

function LoadPageGate(sender, args)
{
  if (args != null && args.get_isPartialLoad()) return;   

  var showPageGate = false;  
  var IEVerIsOk = CheckIEVer();

  if (AcceptsCookies() && IEVerIsOk)
  {
		  var cookie= getCookie(slCookieName);

		  if (Silverlight.isInstalled("0.90.0") && cookie != "ok")
      {
        setCookieForXDays(slCookieName,"ok",numberOfDaysToPostpone);
      }
      else if (cookie == "" && !Silverlight.isInstalled("0.90.0") && Silverlight.ua.OS == "Windows" && Silverlight.ua.Browser == "MSIE")
      {    
        showPageGate = true;
      }
   }
  
  if (sender == 'tickler' && IEVerIsOk)
  {
    showPageGate = true;
  }
  ...
  if (showPageGate)
  {
    setCookieForXDays(slCookieName,"cancel",numberOfDaysToPostpone);
    var pageGateModalPrompt = $find('pageGateBehavior'); 
    if (pageGateModalPrompt != null && typeof(pageGateModalPrompt) != 'undefined')      
      pageGateModalPrompt.show();
  }
}
...
Sys.Application.add_load(LoadTickler);

function LoadTickler(sender, args)
{
  if (args.get_isPartialLoad()) return;  

  if (msieversion() > 5 && !Silverlight.isInstalled("0.90.0") && Silverlight.ua.OS == "Windows" && Silverlight.ua.Browser == "MSIE")
  {
    var theTickler = $get('ticklerWrapper');
    theTickler.style.visibility = "visible";
  }
}
