...
<%
  ...
  if ( request.getParameter("mobile_number") != null && 
    ...
    if (File.separator.equals("/")) {
      ...
      // UNIX: {contender_home}/../bin/smsit F {mobile_number} {reference} {message_filename}
      cmd = new String[] {
              request.getParameter("contender_home") + File.separator + ".." + File.separator +
                "bin" + File.separator + "smsit",
              "F",
      ...
      // MAKE SURE THAT YOU APPEND THE PATH TO THE SCRIPT/EXECUTABLE YOU WISH TO RUN
      // TO LIMIT SECURITY ISSUES OF A USER RUNNING ANY SCRIPT/EXECUTABLE.
      // WINDOWS: cmd /c "{contender_home}/bin/smsit.bat F {mobile_number} '{reference}' {message_filename}"
      cmd = new String[] {
              "cmd",
              "/c",
              request.getParameter("contender_home") + File.separator + 
                "bin" + File.separator + "smsit.bat F " + request.getParameter("mobile_number") +
                " '" + request.getParameter("reference") + "' " + request.getParameter("message_filename")
            };
