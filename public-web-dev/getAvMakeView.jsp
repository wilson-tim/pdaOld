...
<form action="publicaccess.jsp" method="post">
    ...
    <div class="UserInput">
        <label class="input-name">Vehicle Make</label> <select name="vehicle_make" >
        <c:set var="cval"><%= incidentBean.getVehicle_make() %></c:set>
        <x:forEach var="n" select="$AVMakes//vehicles/element">
                <option value="<x:out select="$n/vehicle_make"/>"
                <x:if select="$n/.[vehicle_make = $cval]">
                    selected
                </x:if>
                ><x:out select="$n/vehicle_make" /></option>
        </x:forEach> 
        </select>
    </div>
    <div class="Controls">
