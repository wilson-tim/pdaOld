...
<form action="publicaccess.jsp" method="post">
    ...
    <div class="UserInput">
        ...
        <div class="Street">
        <x:forEach var="n" select="$SiteItems//items/element[item_category!='STREET']">
                <input type="radio" name="item_combo" value="<x:out select="$n/item_ref"/>|<x:out select="$n/feature_ref"/>|<x:out select="$n/contract_ref"/>"
                <x:if select="$n/.[item_ref = $item_ref]">
                    checked="checked"
                </x:if>
                ><x:out select="$n/public_desc" /></input><br />
        </x:forEach> 
        </div>
        <div class="Property">
        <x:forEach var="n" select="$SiteItems//items/element[item_category='STREET']">
