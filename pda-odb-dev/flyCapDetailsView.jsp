...
<%@ page import="com.vsb.flyCapDetailsBean, com.vsb.itemLookupBean, com.vsb.itemDetailBean" %>
<%@ page import="com.vsb.helperBean, com.vsb.recordBean" %>
...
<body onUnload="">
  <form onSubmit="return singleclick();" action="flyCapDetailsScript.jsp" method="post">
    <table width="100%">
      <tr>
      ...
      <tr><td><font color="#ff6565"><b><jsp:getProperty name="flyCapDetailsBean" property="error" /></b></font></td></tr>
      <tr><td><hr size="2" noshade="noshade" /></td></tr>
    </table>
    <table width="100%">
        ...
        <td>
          <select name="land_type" size="4">
            ...
            <sql:resultSet id="rset">
              ...
              <if:IfTrue cond='<%= ((String)pageContext.getAttribute("lookup_code")).trim().equals(flyCapDetailsBean.getLand_type()) %>' >
        ...
        <td>
          <select name="load_size" size="4">
            ...
            <sql:resultSet id="rset">
              ...
              <if:IfTrue cond='<%= ((String)pageContext.getAttribute("load_ref")).trim().equals(flyCapDetailsBean.getLoad_size()) %>' >
                <option value="<sql:getColumn position="1" />" selected="selected" >
      ...
      <tr>
        <td>
          <b>Quantity</b>&nbsp;
          <if:IfTrue cond='<%= flyCapDetailsBean.getLoad_qty().equals("") %>' >
            <input type="text" name="load_qty" size="4" value="" />
          </if:IfTrue>
          <if:IfTrue cond='<%= ! flyCapDetailsBean.getLoad_qty().equals("") %>' >
            <input type="text" name="load_qty" size="4"
              value="<jsp:getProperty name="flyCapDetailsBean" property="load_qty" />" />
          </if:IfTrue>
        </td>
      </tr>
      <tr><td><hr size="1" noshade="noshade" /></td></tr>
        ...
        <td>
          <select name="dom_waste_type" size="4">
            ...
            <sql:resultSet id="rset">
              <sql:getColumn position="1" to="lookup_code" />
              <sql:getColumn position="2" to="lookup_text" />
              <if:IfTrue cond='<%= ((String)pageContext.getAttribute("lookup_code")).trim().equals(flyCapDetailsBean.getDom_waste_type()) %>' >
                <option value="<sql:getColumn position="1" />" selected="selected" >
                  <%= helperBean.restrict(((String)pageContext.getAttribute("lookup_text")).trim(),25) %>
                </option>
              </if:IfTrue>
      ...
      <tr>
        <td>
          <b>Quantity</b>&nbsp;
          <if:IfTrue cond='<%= flyCapDetailsBean.getDom_waste_qty().equals("") %>' >
            <input type="text" name="dom_waste_qty" size="4" value="" />
          </if:IfTrue>
          <if:IfTrue cond='<%= ! flyCapDetailsBean.getDom_waste_qty().equals("") %>' >
            <input type="text" name="dom_waste_qty" size="4"
              value="<jsp:getProperty name="flyCapDetailsBean" property="dom_waste_qty" />" />
          </if:IfTrue>
        </td>
      </tr>
      </sql:statement>
    ...
    <if:IfTrue cond='<%= recordBean.getAction_flag().equals("W") %>' >
      <jsp:include page="include/back_waste_wo_buttons.jsp" flush="true" />
    </if:IfTrue>
    <if:IfTrue cond='<%= ! recordBean.getAction_flag().equals("W") %>' >
      <jsp:include page="include/back_waste_enforce_addtext_buttons.jsp" flush="true" />
    </if:IfTrue>
    <%@ include file="include/insp_sched_buttons.jsp" %>
    <jsp:include page="include/footer.jsp" flush="true" />
    <input type="hidden" name="input" value="flyCapDetails" /> 
