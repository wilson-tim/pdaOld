...
<%@ page import="com.vsb.enfSuspectMarketTradersBean, com.vsb.recordBean, com.vsb.helperBean" %>
...
<head>
  ...
  <title>enfSuspectMarketTraders</title>
  <style type="text/css">
...
<body onUnload="">
  <form onSubmit="return singleclick();" action="enfSuspectMarketTradersScript.jsp" method="post">
    <%-- Title --%>
    <table width="100%">
      <tr>
        <td><h2>Select Suspect</h2>
        </td>
      </tr>
      <tr><td><hr size="2" noshade="noshade" /></td></tr>
      <tr><td><font color="#ff6565"><b><jsp:getProperty name="enfSuspectMarketTradersBean" property="error" /></b></font></td></tr>
      <tr><td><hr size="2" noshade="noshade" /></td></tr>      
    </table>

    <%-- If not empty --%>
    <if:IfTrue cond='<%= enfSuspectMarketTradersPagingBean.getRecordCount() != 0 %>' >
    ...
    <table width="100%">
      ...
      <if:IfTrue cond='<%= enfSuspectMarketTradersPagingBean.getRecordCount() != 0 %>' >
        <tr>
          <td colspan="4">
            <span class="subscript">records <%= enfSuspectMarketTradersPagingBean.getMinRecordInPage() %> - <%= enfSuspectMarketTradersPagingBean.getMaxRecordInPage() %> of <%= enfSuspectMarketTradersPagingBean.getRecordCount() %></span>
          </td>
        </tr>
      </if:IfTrue>
    ...
    <if:IfTrue cond='<%= enfSuspectMarketTradersPagingBean.getRecordCount() != 0 %>' >
      <table width="100%">
        <%-- Trader List start --%>
        <% for (int i = enfSuspectMarketTradersPagingBean.getMinRecordInPage(); i <= enfSuspectMarketTradersPagingBean.getMaxRecordInPage(); ++i){ %>
          <c:set var="i"><%= i %></c:set>
          ...
          <tr bgcolor="<%= bgcolour %>">
            <td valign="top" width="10%">
              <if:IfTrue cond='<%= trader_ref.equals(enfSuspectMarketTradersBean.getTrader_ref()) %>' >
    ...
    <input type="hidden" name="page_number" value="<%= enfSuspectMarketTradersPagingBean.getCurrentPageNum() %>" />
    <input type="hidden" name="input" value="enfSuspectMarketTraders" />
