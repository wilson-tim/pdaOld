...
public class mappingUtils {
  ...
  public String OSRefToLatLng(double easting, double northing) {
    ...
    // This function works in OSGB36 lat/lng format so need to convert the output to WGS84 format
    return OSGB36ToWGS84(OSRefLat, OSRefLng);
  }

  public String LatLngToOSRef(double latWGS84, double lngWGS84) {
    // This function uses an OSGB36 format lat/lng so need to convert the
    // WGS84 format lat/lng that is passed in.
    String OSGB36LatLng = WGS84ToOSGB36(latWGS84, lngWGS84);
    StringTokenizer st = new StringTokenizer(OSGB36LatLng, ",");
    double lat = (new Double(st.nextToken())).doubleValue();
    double lng = (new Double(st.nextToken())).doubleValue();
    ...
    String eastNorth = String.valueOf(E) + "," + String.valueOf(N);       
